...
发行构建使用 Tauri updater：启动后检查、每 6 小时轮询、**发现后后台静默下载**、**用户确认后**重启安装。
...
| 每 6 小时 | 同上 |
| 发现更新 | Toast「发现新版本」；下载中关于页显示进度 |
| 下载完成 | 顶栏横幅 + 关于区副标题「更新已就绪」+「重启」；自动检查下完时 Toast 可带「重启」 |
| 手动「检查」无更新 | Toast「暂无可用更新」 |
| 手动检查失败 | Toast + 关于区显示失败原因（不再伪装成 idle） |
| 用户确认「重启」 | 停托管进程 → 安装 → 重启 |
