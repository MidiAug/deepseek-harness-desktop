...
> 状态：B6 — 设置即时落盘；关于区分展示版本并可检查/更新 harness。
...
| 文件 | 内容 |
|------|------|
| `settings.json` | 镜像、代理、`DSH_HOME` 覆盖、关闭行为 |
| `ui.json` | 顶栏颜色、简洁模式 |

旧版若只有单文件 `settings.json`（含外观字段），首次加载会拆写到两文件。

顶栏中区显示连接状态（含端口）；debug 默认可为 **3081**，正式包为 **3080**（占用时自动顺延）。
...
| 顶栏颜色 | **黑色（#1b1b1c）**（默认）/ **灰色**；高度 **35px** 占位（DSH 在下方）；简洁模式下此项不可用 |
