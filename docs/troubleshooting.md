...
- 是否误开了其它 Harness 桌面壳占端口；关掉冲突实例后再开  
...
- 关于页更新日志应持续出现 npm 行或「仍在执行」心跳；也可打开 `%LOCALAPPDATA%\com.deepseek.harness.desktop\logs\current\shell.log`  
...
- 完整记录仍写入 `%LOCALAPPDATA%\com.deepseek.harness.desktop\logs\current\`（`shell.log` + `harness.log`；历史会话在 `logs\archive\`）  
